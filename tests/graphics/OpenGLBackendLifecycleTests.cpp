...
public:
    ...
    OpenGLDeviceInfo device{"Fake Vendor", "Fake Renderer", "Fake Driver", "Fake GLSL"};
...
ChunkUpload makeUpload(std::uint64_t id, std::uint32_t count = 2U) {
    ...
    upload.metadata.schema.mask = static_cast<std::uint32_t>(PointAttribute::Position);
    upload.cpuData.positions.resize(count);
    for (std::uint32_t index = 0U; index < count; ++index) {
        upload.cpuData.positions[index] = glm::vec3(static_cast<float>(index), 0.0F, 0.0F);
    }
    return upload;
}

std::unique_ptr<OpenGLBackend> makeBackend(
    const std::shared_ptr<FakeContextOperations>& context,
...
void testInitializationFailures() {
    ...
    assertError(backend->init(config()), OpenGLBackendErrorCode::ContextUnavailable);
    context->current = true;
    context->loadResult = false;
    assertError(backend->init(config()), OpenGLBackendErrorCode::FunctionsUnavailable);

    context->loadResult = true;
    capabilities->version = {4, 4, true};
    assertError(backend->init(config()), OpenGLBackendErrorCode::CapabilityCheckFailed);
...
void testUpdateValidationAndThread() {
    ...
    assertError(backend->resize(RenderSize{}), OpenGLBackendErrorCode::ResizeFailed);
...
int main(int argc, char** argv) {
    if (argc > 1 && std::string(argv[1]) == "--real-context") {
        std::cout << "SKIPPED: real OpenGL Context infrastructure is not available in GL-007.\n";
        return 77;
    }
