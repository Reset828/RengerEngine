...
10. **不补全待定项**：已确认的透视轨道交互按相机设计实施；基准硬件、低帧率百分位和性能相机路径继续保持待确定。
...
| ADR-014 | 相机采用已确认的后端无关透视轨道规则；性能路径与基准参数仍待确定 |
...
Camera 模块提供视图矩阵、投影矩阵、相机位置或等价状态、视锥体、窗口尺寸、抽象输入和重置请求。具体控制模型已确认是后端无关的透视轨道相机：左键受约束轨迹球旋转、右键按屏幕平面平移、滚轮固定倍率缩放、Bounds 自动框选和动态 near/far，完整规则见 `cameraInteractionDesign.md`。

Camera 保持在 `engine_core`/`src/camera`，不依赖 Qt、OpenGL、Vulkan 或 CUDA。Qt 到 `InputEvent` 的实际映射、Engine 对具体 Controller 的所有权和命令转发仍是后续集成任务；CA-006 仅实现已确认 Controller。`frustum()` 的裁剪深度范围必须随实际渲染后端与投影矩阵显式决定，不能由参考源码的 Vulkan 实现隐式决定。
...
性能测试使用独立 CMake Target 或 CTest 标签，记录硬件、驱动、数据集、后端、分辨率和运动路径。透视轨道交互已经确认，但可重复性能路径仍待 CA-006 实现和基准环境确认，不得声称已有最终性能场景。
...
等待 CA-006 实现已确认的透视轨道 Controller 后，再结合性能基准环境确定可重复运动路径。
