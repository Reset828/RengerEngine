...
1. 校验状态和调用线程；
2. 批量取出命令并执行合并；
3. 以 `tryPopCompletion()` 或 `tryPopCompletionBatch()` 非阻塞应用数据集任务完成消息，并由 Engine 按 DatasetId 过滤旧结果；
...
- 连续参数命令按“最后一个值生效”合并；
- 加载、取消、卸载、输入和关闭命令不得跨语义重排；`SubmitInputCommand` 是队列屏障，保持 FIFO，不参与参数命令合并；
- `ResetViewCommand` 只调用未来注入的相机控制器入口，当前不定义结果行为。
- `SubmitInputCommand` 由 Engine 校验并消费；QT-005 阶段不注入或调用 `ICameraController`，因此输入命令当前不改变相机状态。
...
空路径、无效颜色、非有限或越界参数、非正视口、非法滚轮值和未支持输入同步返回参数错误，且不得调用命令端口；端口返回的队列满、关闭或 Engine 状态错误原样返回。Snapshot 通过不可变 `std::shared_ptr<const EngineSnapshot>` 获取，Event 通过显式批量轮询获取。
...
TS-003 的 `CommandCoalescer` 为命令队列提供同等的固定容量、FIFO、立即返回和关闭排空语义，但不修改 `BoundedQueue<T>` 的既有接口。它在一把互斥量内扫描最后一个屏障之后的命令段：点大小、着色、固定颜色、CUDA 模式和 Resize 的同类命令保留首次位置并更新为最后值；加载、取消、卸载、背景色、重置视图和 Shutdown 为不可合并屏障。队列已满时，若当前段存在同类可合并命令，仍可原位更新；否则立即返回失败，不阻塞、不淘汰其他命令。`ShutdownCommand` 仅是可消费消息，不自动调用队列 `close()`。`SubmitInputCommand` 已接入，并作为屏障保持 FIFO；输入事件由 Engine 消费但当前不转发 Camera Controller。
...
private:
    // 仅供 CancellationSource 和 TaskSystem 组合观察令牌。
};
...
IO-006 的 `PlyReader final` 与 `PcdReader` 共用公共 Reader 契约，公共头仅包含既有 dzc 类型并通过 Pimpl 隔离 `pcl::PLYReader` 与 `pcl::PCLPointCloud2`。`open()` 先调用 `PLYReader::readHeader()`，再对原始文件从开头只读扫描至 `end_header`；该预校验用于避免 PCL 将不完整 `red`/`green`/`blue` 归并为 `rgb` 后丢失原始声明语义。只接受 `ascii` 与 `binary_little_endian`；其他格式、缺失头、PCL 解析失败、点数溢出和字段校验失败统一返回 `DataFormat/2`。PLY 属性名严格区分大小写，只映射 vertex 元素中的唯一数值标量 `x/y/z`、唯一 uint8 `red/green/blue` 与可选唯一 uint8 `alpha`、唯一数值标量 `intensity`；未知 vertex 标量属性和非 vertex 元素不进入 schema。完整颜色经预校验后使用 PCL 归并的 `rgb`/`rgba` 字段声明 Color；Bounds 与 IntensityMetadata 保持默认值。

IO-005 的 `PcdReader::readNext()` 在首次有效调用时惰性、全量读取 PCD 数据体并私有转换为 SoA；它不是流式 PCL 读取。读取优先级固定为：未打开 `Task/1`，零 `maximumPoints` 为 `Configuration/1`，已取消 Token 为 `Task/7`，然后才处理已缓存的格式失败或数据体加载。PCL 的单次读体调用不可中断，因此在其前后与逐点转换中检查取消；取消不返回部分批次、不推进已对外可见的游标，后续使用未取消 Token 可从首批重新读取/转换。

IO-007 将同一批次策略应用到 `PlyReader`：首次有效 `readNext()` 调用 `PLYReader::read()` 全量读取，再在 `dzc_data_pcl` 内转换并缓存 positions、`0xRRGGBBAA` colors 和量化 intensity，后续仅从私有缓存返回至多 `maximumPoints` 的 `PointBatch`。读体结果必须与 `open()` 已确认的 PLY Header 一致：实际 `width × height` 等于声明点数，`x/y/z`、已确认的 packed `rgb`/`rgba` 和 intensity 字段唯一且类型匹配，点记录步长、字段偏移、行步长与数据区长度均安全一致。读体、字段、长度、转换和 PCL 异常均转换为稳定 `DataFormat/2`，并在关闭前重复返回同一错误。

PLY XYZ 接受数值标量并转为 `glm::dvec3`；任一坐标为 NaN/Infinity 的点跳过，非空声明云若全部无效则失败，0 点云直接 EOF。仅完整的原始 RGB/RGBA 声明可生成 Color：PCL 的 `rgb` 输出按 `0xRRGGBBAA` 规范化且补不透明 alpha，`rgba` 保留 alpha。Intensity 仅为有效坐标点收集，使用一次全文件有效范围量化，确保跨批次可比较；非有限 intensity 写 0 但不剔除该点。成功转换后每个输出批次调用 `validate()`，EOF 后持续成功返回空 `optional`。取消优先于 EOF、终态错误和加载；在 PCL read 后及逐点转换期间检测取消，取消不提交缓存、不返回部分批次、不推进游标，后续未取消调用从首批重新转换。
...
CA-007 已提供从默认 OrbitCameraController 开始的内存 InputEvent 确定性回放，用绝对时间戳驱动逐步 update 并采样状态/矩阵；它只验证相机路径可重复，不采集 FPS。基准 CPU/GPU、驱动、数据集和低帧率百分位阈值仍待确定，因此不能宣称完成最终 `AC-P2-011`。
...
CA-005 至 CA-007 已确定并实现 OrbitCameraController 的抽象输入、轨迹球、reset、裁剪和内存确定性回放。仍待后续任务确认并实现 Engine 注入/所有权/命令转发、Qt 映射以及 Renderer 性能采集；不得据此虚构键位或正式 FPS 结果。

### 29.2 基准环境与性能阈值

仍需确认：CPU、GPU、显存、内存、存储、Windows 版本、驱动、CUDA 版本、正式数据集和低帧率百分位阈值。CA-007 已解决内存相机路径确定性，但当前已确认的 Phase 2 强制指标仍为 1920×1080 持续运动场景平均不低于 30 FPS，目标 60 FPS；最终验收必须在基准环境、数据集和 Renderer 路径确定后执行。
