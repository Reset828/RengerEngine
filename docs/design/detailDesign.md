...
默认单文件最大 20 MiB，保留最近 10 个文件。达到上限后关闭、按序号轮转并重新创建；轮转失败时继续写当前可用 Sink 或 stderr，不能因日志失败终止渲染。记录内容不得逐点输出。多线程先格式化结构化记录，再由日志线程串行写入；队列满时可丢弃低级别记录，但 Error/Fatal 必须同步写备用 Sink。
