...
- **任务状态同步**：完成任务清单中的子任务后，必须同步更新对应任务文档的实现状态、交接记录以及必要的总体进度状态；未满足模块完成规则时，不得将模块标记为完成。
- **主人验证门禁**：后续实现任务中，代码实现与任务完成必须分开记录。代码实现后统一记录为“已实现，待主人验证”，不得仅因代码修改或静态检查通过而勾选任务或宣称完成；只有主人明确确认自行手工运行程序并验证通过后，才可标记为“已完成”。
- **长期手工验证规则**：从 QT-011 本次调整起，以及 QT-012 及之后的所有新任务，默认不新增专门的自动化测试文件。主人自行运行程序并进行交互验证；除非主人另行明确要求，不以新增单元测试、UI 测试或集成测试作为交付内容。此规则不追溯删除或修改 QT-001 至 QT-010 已存在的历史测试文件。
- **编译测试责任**：后续任务默认禁止 AI 执行 CMake configure、CMake build、CTest、单元测试、UI 测试、集成测试，以及为测试复制或整理 Qt/PCL/VTK 等运行时 DLL。主人使用 VS2026/CMake 负责配置、构建和手工程序验证，AI 只提供实现、文档、交接说明和不编译不运行测试的静态检查结果。
- **失败修复流程**：主人报告编译、启动或交互验证失败后，AI 仅依据提供的错误信息和日志修复代码；修复完成后仍记录为“待主人重新验证”，不得代报验证通过。
- **交付说明**：每个实现任务交付时必须明确列出修改文件、实现边界、未由 AI 执行的编译/测试、主人需要验证的事项以及已知限制或风险；并必须提供具体交互步骤、每一步预期可见变化、失败/异常反馈和当前不会发生的行为及原因。不得编造或沿用未经主人确认的当前测试通过结论。
...
*   **接口封装**：
    *   **头文件 (.h)**：仅声明公共接口及必要的公共数据结构。所有辅助函数、工具类实现必须**隐藏在源文件 (.cpp) 中**（推荐使用 Pimpl 模式或无名命名空间）。
...
- **测试 DLL 必须来自仓库根目录 `dll` 文件夹**：每次构建或运行测试程序前，必须先根据目标配置（Debug/Release）和实际二进制依赖检查测试可执行文件、Qt 平台插件（例如 `qoffscreen.dll`）及其非系统运行时依赖，并在仓库根目录的 `dll` 文件夹中核对所需的全部 DLL。确认齐全后，才可以将这些 DLL 复制到对应的构建测试文件夹并执行测试。如果 `dll` 文件夹缺少任何所需 DLL，必须立即停止当前步骤，向主人明确报告每个缺失 DLL、对应配置和用途；必须等待主人将缺失 DLL 添加到 `dll` 文件夹后才能继续，不得从 Qt 安装目录、PATH、其他构建目录或系统开发环境复制/绕过。
...
- 本节适用于本项目所有后续任务，不限于 Qt Application。
- AI 不执行编译和测试；主人在 VS2026/CMake 中自行选择 Kit、生成器、配置、构建目录和测试筛选方式。
- AI 可以读取源码、搜索依赖、检查文档、执行静态分析和 `git diff --check`，但这些检查不能替代主人编译测试。
- 主人明确反馈测试通过后，AI 才能同步勾选任务、更新“已完成”状态和写入当前交接结果；此前只能记录“已实现，待主人验证”。
