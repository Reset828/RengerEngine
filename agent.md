...
- **主人验证门禁**：后续实现任务中，代码实现与任务完成必须分开记录。代码实现后统一记录为“已实现，待主人验证”，不得仅因代码修改或静态检查通过而勾选任务或宣称完成；只有主人明确确认自行编译测试通过后，才可标记为“已完成”并进入下一个功能。
- **编译测试责任**：后续任务默认禁止 AI 执行 CMake configure、CMake build、CTest、单元测试、UI 测试、集成测试，以及为测试复制或整理 Qt/PCL/VTK 等运行时 DLL。主人使用 VS2026/CMake 负责配置、构建和测试，AI 只提供实现、文档、交接说明和不编译不运行测试的静态检查结果。
- **失败修复流程**：主人报告编译或测试失败后，AI 仅依据提供的错误信息和日志修复代码；修复完成后仍记录为“待主人重新验证”，不得代报测试通过。
- **交付说明**：每个实现任务交付时必须明确列出修改文件、实现边界、未由 AI 执行的编译/测试、主人需要验证的事项以及已知限制或风险；不得编造或沿用未经主人确认的当前测试通过结论。
