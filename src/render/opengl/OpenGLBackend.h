...
public:
  ...
  const std::optional<dzc::Error> &lastError() const noexcept {
    return m_lastError;
  }
  OpenGLBackendState state() const noexcept { return m_state; }
  bool isInitialized() const noexcept {
    return m_state == OpenGLBackendState::Initialized;
  }
  std::size_t chunkCount() const noexcept { return m_chunks.size(); }
  bool hasChunk(dzc::ChunkId chunkId) const noexcept;
  const std::optional<dzc::RenderFrame> &currentFrame() const noexcept {
    return m_currentFrame;
  }
  const dzc::RenderSize &renderSize() const noexcept { return m_renderSize; }
  std::uint64_t drawCount() const noexcept { return m_drawCount; }
  std::uint64_t submittedPointCount() const noexcept {
    return m_submittedPointCount;
  }
...
private:
  ...
  dzc::Result<void> makeFailure(OpenGLBackendErrorCode code,
                                const char *userMessage, std::string diagnostic,
  ...
  dzc::Result<void> resetDrawResources();
